...
Connected Secondary Account(s): """)
    total_secAccs = len(bankAccount.secondaryAcc)
    for i in range(1,total_secAccs):
        print(f"{bankAccount.secondaryAcc[i].accountName}")
...
Current Balance: {sa.balance}""")
        ...
        sa.receive(amount)
        total_secAccs = len(ba.secondaryAcc)
        ba.balance -= amount*total_secAccs
        print(f"""Primary Account Name: {ba.accountName}
    Current Balance: {ba.balance}
Secondary Account Name: {sa.accountName}
    Transfer Limit: {sa.transferLimit}
    Current Balance: {sa.balance}""")
        sys.exit(0)
